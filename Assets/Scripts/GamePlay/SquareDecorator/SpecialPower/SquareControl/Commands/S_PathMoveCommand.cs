...
public class S_PathMoveCommand : SquareCommand
{
    ...
    public void TryGetSwapTarget(E_CustomDir dir) 
    {
        if (controlSquare != null)
        {
            ...
            if (RayChecker.CheckTargetLayerObj(LayerMask.GetMask("Player"), 0.5f, controlSquare.transform.position, dir) != null)
                targetSquare = RayChecker.CheckTargetLayerObj(LayerMask.GetMask("Player"), 0.5f, controlSquare.transform.position, dir).GetComponent<Square>();
    ...
    public override void Excute()
    {
        ...
        if(targetSquare!=null &&targetSquare.GetComponent<PlayerController>())
        {

            PostProcessManager.Instance.GrayWorldFlash();
            PostProcessManager.Instance.LenDistortionFlash(0, 1, 0.1f);
            EventCenter.Instance.EventTrigger(E_EventType.E_GetSquareRemoveScore, -300);

            //controlSquare.removeTime--;
            if (controlSquare.removeTime<=1)
            mono.StartCoroutine(controlSquare.BeRemoved());
            return;
        }
